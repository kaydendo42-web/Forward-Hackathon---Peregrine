...
import { addPlanningRequest, logEvent, receiveEvidence, reconcile, reviewRequest, rewordRequest } from '../core/workflow';
...
export const PROMPT_VERSION = 'assist-2';
...
export const assistContextSchema = z.object({
  ...
  request: z.object({
    ...
    reconciliation: z.object({ evidenceTotalAud: aud.nullable(), differenceAud: aud.nullable() }).optional(),
...
export function buildContext(request: CollectionRequest, baseline: Baseline, text?: string): AssistContext {
  ...
  const totals = reconcile(request);
  return {
    ...
    request: {
      ...
      reconciliation: { evidenceTotalAud: money(totals.evidenceCents), differenceAud: money(totals.differenceCents) },
...
const TASK_PROMPTS: Record<AssistTask, string> = {
  follow_up: 'Draft ONE concise adviser follow-up message for the client about this request: what is still missing or inconsistent and exactly what to send. The app-calculated request.reconciliation compares current-year evidence with comparisonAud; a nonzero differenceAud is unresolved and MUST be stated in the message with a request for supporting records or clarification. Never say no further action is needed when a difference remains. Null means unknown, not zero. Prior-year amounts do not close a current-year gap. Even a zero difference requires adviser review. Plain language, no jargon, under 120 words. Items: {"text": string, "basis": string[]}.',
