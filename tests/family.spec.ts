...
test('one warm family email covers every entity with outstanding items', async ({ page }) => {
  ...
  await expect(page.locator('.feedback .notice')).toContainText('Family email drafted for Alex Taylor');
  ...
  await expect(draft).toContainText('Family group · one email to Alex Taylor');
  await expect(draft.locator('pre')).toContainText('Hi Alex,');
